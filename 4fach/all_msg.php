...
//    if ($_SESSION [flt_gelesen]  != 1){$readwhat = " NOT ";} else {$readwhat = " ";}
    ...
    if (isset ($_SESSION["am_flt_search"])) {
      $query_search = "(".
          "(".$conf_4f_tbl ["nachrichten"].".`04_nummer` LIKE \"%".$_SESSION["am_flt_search"]."%\") OR ".
          "(".$conf_4f_tbl ["nachrichten"].".`10_anschrift` LIKE \"%".$_SESSION["am_flt_search"]."%\") OR ".
          "(".$conf_4f_tbl ["nachrichten"].".`12_abfzeit` LIKE \"%".$_SESSION["am_flt_search"]."%\") OR ".
          "(".$conf_4f_tbl ["nachrichten"].".`12_inhalt` LIKE \"%".htmlentities ($_SESSION["am_flt_search"])."%\") OR ".
          "(".$conf_4f_tbl ["nachrichten"].".`13_abseinheit` LIKE \"%".$_SESSION["am_flt_search"]."%\") )";
...
//      unset ($_SESSION["flt_search"]);
    ...
    if ( $_SESSION["am_flt_darstellung"] == "1" ){
      ...
      $anzahl = $tmp[0];
      $_SESSION["am_flt_rescount"] = $anzahl ;
...
\******************************************************************************/
  ...
  function  listen_navi (){
    include ("../4fcfg/config.inc.php");
    echo "<input type=\"image\" name=\"am_flt_start\" src=\"".$conf_design_path."/go_start.gif\" alt=\"Anfang\">\n";
    echo "<input type=\"image\" name=\"am_flt_back\"  src=\"".$conf_design_path."/go_back.gif\" alt=\"zurueck\">\n";
    echo "<input type=\"image\" name=\"am_flt_for\"   src=\"".$conf_design_path."/go_forward.gif\" alt=\"vor\">\n";
    echo "<input type=\"image\" name=\"am_flt_end\"   src=\"".$conf_design_path."/go_end.gif\" alt=\"Ende\">\n";
  }
...
\******************************************************************************/
  function darstellungs_art ( ){
    ...
    echo "<td>";
    echo "<big><b>".($_SESSION["am_flt_start"]+1)."|".($_SESSION["am_flt_start"]+$_SESSION["am_flt_anzahl"])."|<big>".($_SESSION["am_flt_rescount"])."</big></b></big>";
    echo "</td>";
    echo "<td>";
    ...
    echo "<td>";
    echo "<div  style=\"border-top-color:#87CEEB; border-left-color:#87CEEB; border-right-color:#87CEEB; border-bottom-color:#000000; border-width:1px; border-style:solid; padding:0px\">";
      for ($pps=5; $pps <=25; $pps+=5){
        if ( $_SESSION["am_flt_anzahl"] == $pps )  {
          echo "<a href=\"".$_SERVER ["PHP_SELF"]."?am_flt_anzahl_x=1&am_flt_anzahl=".$pps."\"><img src=\"button.php?type=icon&status=AUS&text=".$pps."&bg=blue\" border=\"0\" alt=\"Anzahl".$pps."EIN\"></a>";
        } else {
          echo "<a href=\"".$_SERVER ["PHP_SELF"]."?am_flt_anzahl_x=1&am_flt_anzahl=".$pps."\"><img src=\"button.php?type=icon&status=EIN&text=".$pps."&bg=lighterblue\" border=\"0\" alt=\"Anzahl".$pps."AUS\"></a>";
        }
      }
    echo "</div>";
    echo "</td>";
...
/*
    ...
    if ($_SESSION ["am_flt_unerl"] == 0)  {
      echo "<div>";
      echo "<input type=\"image\" name=\"am_flt_unerledigt_ein\" src=\"button.php?type=push&textpos=buttom&status=AUS&text=un-\" alt=\"unerledigte\">\n";
      echo "</div>";
    } else {
      echo "<div>";
      echo "<input type=\"image\" name=\"am_flt_unerledigt_aus\" src=\"button.php?type=push&textpos=buttom&status=EIN&text=un-\" alt=\"unerledigte\">\n";
      echo "</div>";
    }
    echo "</td>";

    echo "<td>";
    if ($_SESSION ["am_flt_erl"] == 0)  {
      echo "<div>";
      echo "<input type=\"image\" name=\"am_flt_erledigt_ein\" src=\"button.php?type=push&textpos=buttom&status=AUS&text=erledigt\" alt=\"erledigte\">\n";
      echo "</div>";
    } else {
      echo "<div>";
      echo "<input type=\"image\" name=\"am_flt_erledigt_aus\" src=\"button.php?type=push&textpos=buttom&status=EIN&text=erledigt\" alt=\"erledigte\">\n";
      echo "</div>";
    }
    echo "</td>";
*/
    echo "<td>";
    if ($_SESSION ["am_flt_find_mask"] == 0)  {
      echo "<div>";
      echo "<input type=\"image\" name=\"am_flt_find_mask_ein\" src=\"button.php?type=push&textpos=buttom&status=AUS&text=finden\" alt=\"finden\">\n";
      echo "</div>";
    } else {
      echo "<div>";
      echo "<input type=\"image\" name=\"am_flt_find_mask_aus\" src=\"button.php?type=push&textpos=buttom&status=EIN&text=finden\" alt=\"finden\">\n";
      echo "</div>";
    }
    echo "</td>";
    ...
    if ($_SESSION["am_flt_find_mask"] == 1){
      echo "\n<form action=\"".$_SERVER ["PHP_SELF"]."\" method=\"get\" target=\"_self\">\n";
      echo "<td>";
      if (isset ($_SESSION ["am_flt_search"]) ) { $defvalue = $_SESSION ["am_flt_search"] ;}
      else {$defvalue = "";}
      echo "<div>";
      echo "<p>Suchbegriff: <input name=\"am_flt_search\" value=\"".$defvalue."\" type=\"text\" size=\"30\" maxlength=\"30\"></p>";
      echo "<div>";
      echo "</td>";
      echo "<td>";
      echo "<input name=\"am_flt_suche\" value=\"suchen\" type=\"submit\">\n";
      echo "</td>";
    //          echo "</form>";
      echo "</div>";
    }
...
define ("debug", true);
  ...
  \**********************************************************************/
  if (!isset ( $_SESSION["am_flt_darstellung"])){
    $_SESSION["am_flt_darstellung"] = 1;
    $_SESSION["am_flt_erledigt"]    = 0;
    $_SESSION["am_flt_unerledigt"]  = 1;
    $_SESSION["am_flt_anzahl"]      = 15;
    $_SESSION["am_flt_start"]       = 0 ;
    $_SESSION["am_flt_position"]    = 0;
  }
/*
  // filtern EIN / AUS
  if ( (isset ($_GET["am_flt_darstellung_aus_x"])) or
       (isset ($_GET["am_flt_darstellung_ein_x"])) ){

    if ( ($_SESSION["am_flt_darstellung"] == 1) and (isset ($_GET["am_flt_darstellung_aus_x"])) ) {
      $_SESSION["am_flt_darstellung"] = 0;
    } elseif ( ($_SESSION["am_flt_darstellung"] == 0) and (isset ($_GET["am_flt_darstellung_ein_x"])) ){
      $_SESSION["am_flt_darstellung"] = 1;
    }
  }

  // erledigte SICHTAR UNSICHTBAR
  if ( (isset ($_GET["am_flt_erledigt_aus_x"])) or
       (isset ($_GET["am_flt_erledigt_ein_x"])) ){

    if ( ($_SESSION["am_flt_erledigt"] == 1) and (isset($_GET["am_flt_erledigt_aus_x"])) ) {
      $_SESSION["am_flt_erledigt"] = 0;
    } elseif ( ($_SESSION["am_flt_erledigt"] == 0) and (isset ($_GET["am_flt_erledigt_ein_x"])) ){
      $_SESSION["am_flt_erledigt"] = 1;
    }
  }
  // unerledigte SICHTBAR UNSICHTBAR
  if ( (isset ($_GET["am_flt_unerledigt_aus_x"])) or
       (isset ($_GET["am_flt_unerledigt_ein_x"])) ){

    if ( ($_SESSION["am_flt_unerledigt"] == 1) and (isset($_GET["am_flt_unerledigt_aus_x"])) ) {
      $_SESSION["am_flt_unerledigt"] = 0;
    } elseif ( ($_SESSION["am_flt_unerledigt"] == 0) and (isset ($_GET["am_flt_unerledigt_ein_x"])) ){
      $_SESSION["am_flt_unerledigt"] = 1;
    }
  }
*/
  ...
  if ( (isset ($_GET["am_flt_find_mask_aus_x"])) or
    ...
    if ( ($_SESSION["am_flt_find_mask"] == 1) and (isset($_GET["am_flt_find_mask_aus_x"])) ) {
      unset ($_SESSION["am_flt_search"]);
      $_SESSION["am_flt_find_mask"] = 0;
    } elseif ( ($_SESSION["am_flt_find_mask"] == 0) and (isset ($_GET["am_flt_find_mask_ein_x"])) ){
      $_SESSION["am_flt_find_mask"] = 1;
    }
  }

  if (isset($_GET["am_flt_suche_reset"])){ unset ($_SESSION["am_flt_search"]); }

  if (isset($_GET["am_flt_suche"])){
    if ($_SESSION["am_flt_search"] != $_GET ["am_flt_search"]){
      $_SESSION["am_flt_start"] = 0 ;
      $_SESSION["am_flt_position"] = 0;
    }
    $_SESSION["am_flt_search"] = $_GET ["am_flt_search"];
  }
