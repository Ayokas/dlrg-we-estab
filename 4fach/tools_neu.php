...
\******************************************************************************/
      ...
      $vordoppelt = false; // vorgaenger doppelt
      $tableisset = false;
          // Schleifenanfang Statusdarstellung
      while ( $i <= count ($conf_empf) ){
        ...
        if ( ( strlen( $conf_empf [$i]["fkt"] ) <= 2 ) and
          ...
          echo "<!-- 001a - 472 - list.php -->\n";
           //
                //              if (!$statusalt) { $absneu = true; }  // dann
          $doppel = true;
        } else {
          $doppel = false;
          if (($vordoppelt != $doppel) and ($i >1) )  {
            echo "<!-- 002 -(vordoppelt != doppel) and (i >1) i=".$i." ~ tableisset zu-list.php -->\n";
            echo "</tbody>"; echo "</table>\n";
            $tableisset = false;
          }
        }

        if ( ( $doppel != $vordoppelt) and !($tableisset) ) {
          echo "<!-- 003 -(doppel != vordoppelt) and !(tableisset)- liste.php -->\n";
          echo "<table align=\"center\" style=\"text-align:center; width:".
          $zellenbreite."; height:".$zellenhoehe."; font-size:9pt; background-color: rgb(150, 150, 150);
          font-size:9pt; height:".$zellenhoehe.";\" border=\"0\" cellpadding=\"1\" cellspacing=\"1\">\n";
          echo "<tbody>\n";
          $tableisset = true;
//          $absneu = false;
          $vordoppelt = $doppel;
        ...
        if ($conf_empf[$i]["fkt"] != "A/W")
          if ($doppel) {
            echo "<!-- 004 doppel=".$doppel."---vordoppel=".$vordoppelt."---tableisset=".$tableisset." -->\n";
            echo "<tr>\n";
            ...
            echo "<td style=\"background-color: ".
                 $userstatus [($conf_empf[$i+1]["rolle"])][($conf_empf[$i+1]["fkt"])]."height:".
                 $zellenhoehe."; font-size:9pt; font-weight:bold;\">".$conf_empf[$i+1]["fkt"];
            echo "</td>\n";
            echo "</tr>\n";
            $i += 2;
            $vordoppelt = true ;
            ...
            echo "<!-- 005 liste.php -->\n";
            echo "<tr>\n";
            echo "<td style=\"background-color: ".$userstatus [($conf_empf[$i]["rolle"])][($conf_empf[$i]["fkt"])]."height:".$zellenhoehe."; font-size:9pt; font-weight:bold;\">".$conf_empf[$i]["fkt"] ;
            echo "</td>\n";
            echo "</tr>\n";
            $i ++;
            $vordoppelt = false;
        ...
        if ($conf_empf[$i]["fkt"] == "A/W") {
          ...
          echo "<!-- 006 fkt == A/W - tableisset zu -  liste.php -->\n";
          echo "</tbody>"; echo "</table>\n";
          $tableisset = false;
          echo "<!-- 006a fkt == A/W - tableisset auf -  liste.php -->\n";
...
//            }
          ...
          if  ($i <= (count ($conf_empf)-1))   {
            echo "<table align=\"center\" style=\"text-align:center; width:".$zellenbreite."; height:".$zellenhoehe."; font-size:9pt; background-color: rgb(150, 150, 150);  font-size:9pt; height:".$zellenhoehe.";\" border=\"0\" cellpadding=\"1\" cellspacing=\"1\">\n";
            echo "<tbody>\n";
            echo "<!-- 008 -tableisset auf- liste.php -->\n";
            $tableisset = true;
          }
        } // fkr == A/W
/*
        if ($tableisset){
          echo "<!-- 009 ".$tableisset."- liste.php -->\n";
          echo "</tbody>";
          echo "</table>\n";
          $tableisset = false;
        }
